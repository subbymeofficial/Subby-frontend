...
export interface RegisterPushTokenPayload {
  ...
  // Optional: the backend may want to know which user this token belongs to,
  // but the backend already knows that from the bearer token in the request.
}
...
export const pushService = {
  async registerToken(payload: RegisterPushTokenPayload): Promise<void> {
    try {
      ...
      // Non-fatal — log and move on. Backend may not have the route yet.
