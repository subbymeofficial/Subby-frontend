    ...
    <DashboardLayout title="Contractor Dashboard" navItems={navItems}>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Subscribe to apply for jobs and appear in search results.
              </p>
            </div>
          </div>
          <Link
        ...
        <div className="mb-6 rounded-lg border border-success/30 bg-success/5 p-4 flex items-center gap-3">
          ...
          <div>
            <p className="font-semibold text-foreground capitalize">
              {subStatus?.plan} Plan — Active
            </p>
            <p className="text-sm text-muted-foreground">
              {subStatus?.status === "trialing"
                ? "Founding member — free year active"
                : "Subscription active"}
              {subStatus?.expiresAt &&
                ` · Renews ${new Date(subStatus.expiresAt).toLocaleDateString()}`}
            </p>
          </div>
        </div>
      )}
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <StatsCard
          ...
          change={
            earnings?.pendingEscrow
              ? `$${(earnings.pendingEscrow / 100).toFixed(0)} pending`
              : undefined
          }
          positive
          icon={DollarSign}
      ...
      <div className="mt-8 rounded-lg border bg-card p-6 card-shadow">
        ...
        {/* Primary action row — makes the Welcome card actionable instead of just descriptive */}
        <div className="mt-4 flex flex-wrap gap-2">
          <Button asChild size="sm">
            <Link to="/dashboard/contractor/jobs">
              <Briefcase size={16} className="mr-1" /> Browse Jobs
            </Link>
          </Button>
          <Button asChild variant="outline" size="sm">
            <Link to="/dashboard/contractor/applications">
              <FileCheck size={16} className="mr-1" /> My Applications
            </Link>
          </Button>
          <Button asChild variant="ghost" size="sm">
            <Link to="/dashboard/contractor/profile">
              <UserCog size={16} className="mr-1" /> Edit Profile
            </Link>
          </Button>
        </div>
        ...
        {pendingApps.length > 0 && (
          <div className="mt-4 space-y-3">
            ...
            {pendingApps.length > 3 && (
              <Link
                to="/dashboard/contractor/applications"
